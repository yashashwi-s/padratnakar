...
Canonical headings and verses retain their printed horizontal positions and horizontal alignment. Safe text groups use their individual source boxes; unaligned fragments use the complete accepted line. SVG text lengths fit those groups into the measured boxes without inserting spaces into the corpus. Every canonical verse remains one displayed line. Source page continuations join using the median printed line leading, without book page headers. Canonical verses add 3 source units of line spacing and 8 between the stored stanzas. This includes the three-line opening stanza of pad 54. Shodash retains its separate heading, numbering, stanza-spacing and dedication treatment.

The page includes an additional 12 source units of white margin on each side and fits the available viewport up to a 560 CSS-pixel reading width. There are no text-size or percentage zoom controls. A two-finger gesture scales only the inner page from 1× to 3× without reflow; the app buttons remain fixed. One-finger panning works while enlarged. Swipe-to-next is disabled during enlargement and multi-touch, while navigation buttons always work. A vertical gesture cancels that swipe attempt; horizontal swipes work from any scroll position. Short deliberate flicks use a 28-pixel threshold (40 pixels for slower swipes). Physical-device gesture feel still needs phone verification.

The last heading and first verse use a consistent 60-source-unit baseline gap in both collections. Within canonical verses, original PDF baseline spacing receives the line and stanza additions above.

Headings, verses, raised note markers and footnotes remain Unicode SVG text. Selection and the touch callout are disabled; holding still for 550 ms copies the complete displayed pad. Moving more than 8 pixels cancels copying. Footnote positions and relative sizes come from the source spans. If source geometry cannot load, accepted text remains visible in a simple fixed-line fallback.

The font is still the licensed Unicode Noto Serif Devanagari, not the PDF's legacy ChanakyaBold subsets. Source widths and vertical geometry are followed, but the glyph outlines are not identical to the PDF. This is not a claim of an exact font match or a facsimile. Enlarging text changes the inner page scale rather than line breaking; this is the user's requested tradeoff for preserving the book layout.
